...
const filterFields: Array<[keyof OrderItem | 'count', string]> = [
  ['count', 'Количество'],
  ['total_price', 'Цена'],
]
const groupDefault: keyof OrderItem = 'warehouse_name'
const filterDefault: keyof OrderItem | 'count' = filterFields[0][0]
...
async function getData() {
  if (endpointURL && apiKey && dateFrom.value && dateTo.value) {
    loading.value = true
    error.value = null
    const { data: responseData, error: responseError } = await fetchData<'orders'>(
      endpointURL,
      'GET',
      {
...
function convertData() {
  ...
  if (filter.value === 'count') {
    ...
    chartData.value = convertDataGeneric(
      data.value.data,
      groupDefault,
      filter.value as keyof OrderItem,
      label,
    )
  }
}
