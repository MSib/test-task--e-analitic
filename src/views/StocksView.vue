...
const filterFields: Array<[keyof StockItem | 'count', string]> = [
  ['quantity_full', 'Количество'],
  ['price', 'Цена'],
]
const groupDefault: keyof StockItem = 'warehouse_name'
const filterDefault: keyof StockItem | 'count' = filterFields[0][0]
...
async function getData() {
  if (endpointURL && apiKey && dateFrom.value && dateTo.value) {
    loading.value = true
    error.value = null
    const { data: responseData, error: responseError } = await fetchData<'stocks'>(
      endpointURL,
      'GET',
      {
...
function convertData() {
  ...
  if (filter.value === 'count') {
    ...
    chartData.value = convertDataGeneric(
      data.value.data,
      groupDefault,
      filter.value as keyof StockItem,
      label,
    )
  }
}
